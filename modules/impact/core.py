...
def enhance_detail(image, model, clip, vae, guide_size, guide_size_for_bbox, max_size, bbox, seed, steps, cfg,
    ...
    if control_net_wrapper is not None:
        # Pass VAE for VACE compositing if supported
        try:
            positive, negative, cnet_pils = control_net_wrapper.apply(positive, negative, upscaled_image, noise_mask, vae=vae)
        except TypeError:
...
class VACEWrapper:
    def __init__(self, vace_frames, vace_masks, vace_strength, 
                 original_size, crop_region, seg_mask=None, prev_wrapper=None, 
                 composite_both_channels=True):

        self.vace_frames = vace_frames
        self.vace_masks = vace_masks
        self.vace_strength = vace_strength
        self.original_size = original_size
        self.seg_mask = seg_mask
        self.prev_wrapper = prev_wrapper
        self.composite_both_channels = composite_both_channels
        
        # make cropped region divisible by 16 (for patch embedding compatibility)
        x1, y1, x2, y2 = crop_region
        width = x2 - x1
        height = y2 - y1
        
        adjusted_width = ((width + 8) // 16) * 16
        adjusted_height = ((height + 8) // 16) * 16
        
        adjusted_width = max(16, adjusted_width)
        adjusted_height = max(16, adjusted_height)
        
        # center the adjusted crop within original crop
        width_diff = width - adjusted_width
        height_diff = height - adjusted_height
        
        new_x1 = x1 + width_diff // 2
        new_y1 = y1 + height_diff // 2
        ...
        if new_y1 < 0:
            new_y2 -= new_y1
            new_y1 = 0
        if new_x2 > original_size[1]:
            new_x1 -= (new_x2 - original_size[1])
            new_x2 = original_size[1]
        if new_y2 > original_size[0]:
            new_y1 -= (new_y2 - original_size[0])
            new_y2 = original_size[0]
        
        # check again, adjust again if out of bounds
        final_width = new_x2 - new_x1
        final_height = new_y2 - new_y1
        if final_width % 16 != 0:
            new_x2 = new_x1 + (final_width // 16) * 16
        if final_height % 16 != 0:
            new_y2 = new_y1 + (final_height // 16) * 16
        
        self.crop_region = [new_x1, new_y1, new_x2, new_y2]
        
        self.latent_crop = [
            self.crop_region[0] // 8,
            self.crop_region[1] // 8,
            self.crop_region[2] // 8,
            self.crop_region[3] // 8
        ]

    def doit_ipadapter(self, model):
        if self.prev_wrapper is not None:
            return self.prev_wrapper.doit_ipadapter(model)
        return model, []

    def apply(self, positive, negative, image, mask=None, use_acn=False, vae=None):
        cnet_pils = []
        
        if self.prev_wrapper is not None:
            try:
                positive, negative, cnet_pils = self.prev_wrapper.apply(
                    positive, negative, image, mask, use_acn, vae=vae)
            except TypeError:
                positive, negative, cnet_pils = self.prev_wrapper.apply(
                    positive, negative, image, mask, use_acn)
        
        x1, y1, x2, y2 = self.latent_crop
        
        cropped_frames = [
            vf[:, :, :, y1:y2, x1:x2] 
            for vf in self.vace_frames
        ]
        
        cropped_masks = [
            vm[:, :, :, y1:y2, x1:x2]
            for vm in self.vace_masks
        ]
        
        if self.seg_mask is not None:
            cropped_masks = self._apply_seg_mask_constraint(cropped_masks)
        
        # composite with original (only if seg_mask and vae provided)
        if vae is not None and self.seg_mask is not None:
            cropped_frames = self._composite_masks(cropped_frames, cropped_masks, image, vae)
        ...
        positive = node_helpers.conditioning_set_values(positive, vace_dict, append=False)
        negative = node_helpers.conditioning_set_values(negative, vace_dict, append=False)
        
        return positive, negative, cnet_pils
    
    def _apply_seg_mask_constraint(self, cropped_masks):
        import torch
        import numpy as np
        
        if isinstance(self.seg_mask, np.ndarray):
            seg_mask_tensor = torch.from_numpy(self.seg_mask).float()
        else:
            seg_mask_tensor = self.seg_mask.float()
        
        if not cropped_masks:
            return cropped_masks
        
        vm_h = cropped_masks[0].shape[-2]
        vm_w = cropped_masks[0].shape[-1]
        
        if len(seg_mask_tensor.shape) == 3 and seg_mask_tensor.shape[0] > 1:
            num_frames = seg_mask_tensor.shape[0]
            seg_mask_resized = torch.nn.functional.interpolate(
                seg_mask_tensor.unsqueeze(1),  # [frames, 1, h, w]
                size=(vm_h, vm_w),
                mode='bilinear',
                align_corners=False
            ).squeeze(1)  # [frames, h_lat, w_lat]
        else:
            # Single frame mask [h, w] or [1, h, w]
            if len(seg_mask_tensor.shape) == 2:
                seg_mask_tensor = seg_mask_tensor.unsqueeze(0)
            seg_mask_resized = torch.nn.functional.interpolate(
                seg_mask_tensor.unsqueeze(0),
                size=(vm_h, vm_w),
                mode='bilinear',
                align_corners=False
        ...
        constrained_masks = []
        for vm in cropped_masks:
            vm_batch, vm_channels, vm_frames, _, _ = vm.shape
            
            # expand seg_mask to match dimensions
            if len(seg_mask_resized.shape) == 3 and seg_mask_resized.shape[0] > 1:
                frames_to_use = min(seg_mask_resized.shape[0], vm_frames)
                seg_mask_for_vm = seg_mask_resized[:frames_to_use]
                
                seg_mask_expanded = seg_mask_for_vm.unsqueeze(0).unsqueeze(1)  # [1, 1, frames, h_lat, w_lat]
                seg_mask_expanded = seg_mask_expanded.expand(vm_batch, vm_channels, frames_to_use, vm_h, vm_w)
                
                if vm_frames > frames_to_use:
                    # Pad with last frame
                    last_frame = seg_mask_expanded[:, :, -1:, :, :]
                    padding = last_frame.expand(vm_batch, vm_channels, vm_frames - frames_to_use, vm_h, vm_w)
                    seg_mask_expanded = torch.cat([seg_mask_expanded, padding], dim=2)
            else:
                # Single frame mask
                seg_mask_expanded = seg_mask_resized.unsqueeze(0).unsqueeze(0).unsqueeze(0)  # [1, 1, 1, h_lat, w_lat]
                seg_mask_expanded = seg_mask_expanded.expand(vm_batch, vm_channels, vm_frames, vm_h, vm_w)
            
            constrained = vm * seg_mask_expanded
            constrained_masks.append(constrained)
        
        return constrained_masks
    
    def _composite_masks(self, vace_frames, vace_masks, image, vae):
        import torch
        
        if not vace_frames:
            return vace_frames
        
        target_h_lat = vace_frames[0].shape[-2]
        target_w_lat = vace_frames[0].shape[-1]
        
        target_h = target_h_lat * 8
        target_w = target_w_lat * 8
        ...
        original_latents = vae.encode(image)
        
        composited_frames = []
        for i, vf in enumerate(vace_frames):
            mask = vace_masks[i][:, :32, :, :, :]  # Use first 32 channels of mask
            
            # expand original latents to match VACE frame dimensions
            if len(original_latents.shape) == 4:
                orig_expanded = original_latents.unsqueeze(2).expand(-1, -1, vf.shape[2], -1, -1)
            else:
                orig_expanded = original_latents
            
            if orig_expanded.shape[1] == 16:
                # composite with reactive channels
                if self.composite_both_channels:
                    # composite both inactive and reactive
                    inactive_mask = mask[:, :16, :, :, :]
                    reactive_mask = mask[:, 16:32, :, :, :]
                    
                    composited_inactive = vf[:, :16, :, :, :] * inactive_mask + orig_expanded * (1 - inactive_mask)
                    composited_reactive = vf[:, 16:32, :, :, :] * reactive_mask + orig_expanded * (1 - reactive_mask)
                    composited = torch.cat([composited_inactive, composited_reactive], dim=1)
                else:
                    # FOR TESTING: Only composite reactive channels (does this make any sense)
                    reactive_mask = mask[:, 16:32, :, :, :]
                    composited_reactive = vf[:, 16:32, :, :, :] * reactive_mask + orig_expanded * (1 - reactive_mask)
                    composited = torch.cat([vf[:, :16, :, :, :], composited_reactive], dim=1)
            else:
                # can't composite
                composited = vf
